...
pander(addmargins(table(quiz0831$group, quiz0831$Q1, useNA = "ifany"), margin=1))
```

##### Roper(%)

```{r, Roper in percentage, echo =FALSE}
pander(round(prop.table(addmargins(table(quiz0831$group, quiz0831$Q1, useNA = "ifany"), margin=1), margin=1)[3,]*100, digits=1), caption="Roper")
```
...
pander(addmargins(table(quiz0831$group, quiz0831$Q2, useNA = "ifany"), margin=1))
```

##### ASA(%)

```{r, ASA in percentage, echo = FALSE}
pander(round(prop.table(addmargins(table(quiz0831$group, quiz0831$Q2, useNA = "ifany"), margin=1), margin=1)[3, ]*100, digits=1), caption="ASA")
```
...
pander(addmargins(table(quiz0831$group, quiz0831$Q3, useNA = "ifany"), margin=1))
```

##### D.Huff(%)

```{r, D.Huff in percentage, echo = FALSE}
pander(round(prop.table(addmargins(table(quiz0831$group, quiz0831$Q3, useNA = "ifany"), margin=1), margin=1)[3,]*100, digits=1), caption="D.Huff")
```
...
tbl <- table(quiz0831$group, quiz0831$Q4, exclude = NULL)
colnames(tbl)[6] <- "결석"
pander(tbl[-3, ], caption="THAAD")
pander(chisq.test(tbl[-3, -6], simulate.p.value = TRUE))
```

##### {"매우 공감", "다소 공감" => "공감"}, {"다소 비공감", "매우 비공감" => "비공감"}

```{r, THAAD(3 possible answers), echo = FALSE}
tbl.2 <- table(quiz0831$group, quiz0831$Q4.2, exclude = NULL)
colnames(tbl.2)[4] <- "결석"
pander(tbl.2, caption="THAAD")
```
