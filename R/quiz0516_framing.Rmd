...
# quiz0516$Q4.2 <- quiz0516$Q4
Black <- quiz0516$group == "Black"
Red <- quiz0516$group == "Red"
# quiz0516$Q4.2[quiz0516$group == "Black" & quiz0516$Q4 == "가"] <- "400명은 죽는다"
# quiz0516$Q4.2[quiz0516$group == "Black" & quiz0516$Q4 == "나"] <- "모두 사망 않을 확률 1/3"
# quiz0516$Q4.2[quiz0516$group == "Red" & quiz0516$Q4 == "가"] <- "200명은 살린다"
# quiz0516$Q4.2[quiz0516$group == "Red" & quiz0516$Q4 == "나"] <- "모두 살릴 확률 1/3"
# quiz0516$Q4.2 <- quiz0516$Q4
quiz0516$Q4.2[Red] <- ifelse(quiz0516$Q4[Red] == "가", "200명/400명", ifelse(quiz0516$Q4[Red] == "나", "확률 1/3", NA))
quiz0516$Q4.2[Black] <- ifelse(quiz0516$Q4[Black] == "가", "200명/400명", ifelse(quiz0516$Q4[Black] == "나", "확률 1/3", NA))
str(quiz0516)
```
...
pander(round(prop.table(addmargins(table(quiz0516$group, quiz0516$Q1, exclude = NULL), margin=1), margin=1)[4,]*100, digits=1), caption = "방관자 효과")
```
...
pander(round(prop.table(addmargins(table(quiz0516$group, quiz0516$Q2, exclude = NULL), margin = 1), margin = 1)[4, ]*100, digits = 1), caption = "스탬리 밀그램 : 복종")
```
...
pander(round(prop.table(addmargins(table(quiz0516$group, quiz0516$Q3, exclude = NULL), margin = 1), margin = 1)[4, ]*100, digits = 1), caption = "Power of 3")
```
...
tbl <- table(quiz0516$group, quiz0516$Q4.2, exclude = NULL)
rownames(tbl) <- c("Red(산다)", "Black(죽는다)", "NA")
