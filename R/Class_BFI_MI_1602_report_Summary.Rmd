---
title: "Big Five Personality and Multiple Intelligence : 201602"
author: "coop711"
date: "`r Sys.Date()`"
...
dimnames(BFI.score)<-list(ID = ID, BFI.type.kr)
pander(BFI.score)
```
...
dimnames(BFI.level.kr) <- list(ID = ID, BFI.type.kr)
pander(BFI.level.kr)
```
...
dimnames(BFI.level.kr.2)<-list(ID = ID, BFI.type.kr)
(BFI.na.id <- which(apply(BFI.level.kr.2, 1, function(x)all(is.na(x)))))
(BFI.ok.id <- setdiff(1:68, BFI.na.id))
...
```{r, echo = FALSE}
levels(BFI.MI.full$제2순위) <- levels(BFI.MI.full$제1순위)
levels(BFI.MI.full$제3순위) <- levels(BFI.MI.full$제1순위)
